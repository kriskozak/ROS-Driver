...
{
  ros::NodeHandle pnh("~");
  pnh.param("port", port_, std::string("/dev/ttyACM0"));
  pnh.param("baud_rate", baud_rate_, 115200);
  cmd_vel_sub = nh_.subscribe("/cmd_vel", 10, &Driver::cmd_vel_callback, this);
}
...
void Driver::connect()
{
  ...
  if(ser.isOpen()){
    ROS_INFO("Serial port %s is open at %u baud", port_.c_str(), baud_rate_);
  }else{
    ROS_ERROR("Could not open serial port %s at %u baud",
              port_.c_str(),
              baud_rate_);
  }
}
...
bool Driver::configservice(
  roboteq_motor_controller_driver::config_srv::Request &request,
  roboteq_motor_controller_driver::config_srv::Response &response)
{
  std::stringstream str;
  str << "^" << request.userInput << " " << request.channel
      << " " << request.value << "_ " << "%\clsav321654987";
  ser.write(str.str());
...
bool Driver::commandservice(
  roboteq_motor_controller_driver::command_srv::Request &request,
...
bool Driver::maintenanceservice(
  roboteq_motor_controller_driver::maintenance_srv::Request& request,
...
{
  configsrv = nh_.advertiseService(
    "config_service",
    &Driver::configservice,
    this);

  commandsrv = nh_.advertiseService(
    "command_service",
    &Driver::commandservice,
    this);

  maintenancesrv = nh_.advertiseService(
    "maintenance_service",
    &Driver::maintenanceservice,
    this);
}
...
void Driver::run()
{
  ...
  nh_.getParam("frequencyH",frequencyH);
  nh_.getParam("frequencyL",frequencyL);
  nh_.getParam("frequencyG",frequencyG);

  typedef std::string Key;
  typedef std::string Val;
  std::map<Key,Val> map_sH;
  nh_.getParam("queryH", map_sH);
  ...
  ss0 << "^echof 1_";
  ss1 << "# c_/\"DH:\",\":\"";
  for(std::map<Key,Val>::iterator iter = map_sH.begin();
      iter != map_sH.end();
      ++iter) {
      Key KH =  iter->first;
      //ROS_INFO_STREAM(KH);
  ...
  for (int i = 0; i< KH_vector.size(); i++) {
    publisherVecH.push_back(
      nh_.advertise<roboteq_motor_controller_driver::channel_values>(
        KH_vector[i], 100));
  }

  std::map<Key,Val> map_sL;
  nh_.getParam("queryL", map_sL);
  std::vector<std::string> KL_vector;
  ss2 <<  "/\"DL:\",\":\"";
  for(std::map<Key,Val>::iterator iter = map_sL.begin();
      iter != map_sL.end();
      ++iter) {
    Key KL = iter->first;
  ...
  for(int i=0; i<KL_vector.size(); ++i){
    publisherVecL.push_back(
      nh_.advertise<roboteq_motor_controller_driver::channel_values>(
        KL_vector[i], 100));
  }

  std::map<Key,Val> map_sG;
  nh_.getParam("queryG", map_sG);
  std::vector<std::string> KG_vector;
  ss3 <<  "/\"DG:\",\":\"";
  for(std::map<Key,Val>::iterator iter = map_sG.begin();
      iter != map_sG.end();
      ++iter) {
      Key KG = iter->first;
  ...
  for(int i=0; i<KG_vector.size(); ++i){
    publisherVecG.push_back(nh_.advertise<std_msgs::String>(KG_vector[i],100));
  }
  ser.write(ss0.str());
  ser.write(ss1.str());
  ser.write(ss2.str());
  ...
  while(ros::ok()){
    ...
    loop_rate.sleep();
    ROS_INFO_STREAM("Type the command - \"rostopic list\" - in new terminal "
                    "for publishers");
  }
}
